...
void check_door_action(){
  ...
  switch(door_status){
    case UP: 
      Serial.println("check for closing");
      if(min_after_sunset() == DOOR_CLOSING_OFFSET){
        close_door();
      }
      break;
    case DOWN:
      Serial.println("check for opening");
...
void set_lamp(int intensity){
  if(intensity == -1) {
    ...
    r = map(intensity, 20, 60, 150, 255);
    g = map(intensity, 20, 60, 30, 255);
    b = map(intensity, 21, 60, 0, 100);
  }else{
    r = map(intensity, 0, 20, 20, 150);
    g = map(intensity, 0, 20, 5, 30);
    b = 0;
  }
...
void check_lamp_action(){
  if(min_after_sunset() > LAMP_ON_OFFSET && min_after_sunset() < LAMP_OFF_OFFSET){
